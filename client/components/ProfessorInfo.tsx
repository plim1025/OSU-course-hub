...
import { PROFESSOR_COMMENTS, PROFESSOR_COURSES } from '../utils/graphql';
import { CommentType, Course, Professor } from '../utils/types';
				...
				courses.map((course: Course) => {
					return (
						<Link href={`/course/${course.id}`}>
							<p style={courseList} key={course.id}>
								<b>
...
const ProfessorTags = ({ id }) => {
	...
	let tags: string[] = [];
	comments.forEach((comment: CommentType) => comment.tags.forEach(tag => tags.push(tag)));
...
const ProfessorInfo: React.FC<Props> = ({ professor }) => {
	...
	return (
		<Container className='mt-3'>
			<Card key={professor.id} className='mb-5 p-4 w-75'>
				...
				<h5>
					Based on <b>{quality.length}</b> rating{quality.length == 1 ? null : 's'}.
				</h5>
